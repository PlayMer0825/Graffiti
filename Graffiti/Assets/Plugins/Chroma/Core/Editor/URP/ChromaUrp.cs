...
public class ChromaUrp : BaseShaderGUI {
    ...
    private static BaseGuiType CreateInstance(Shader shader) {
#if UNITY_2021_2_OR_NEWER
        var shaderID = Unity.Rendering.Universal.ShaderUtils.GetShaderID(shader);
        if (_shaderGUI != null && _shaderID == shaderID) return;
        _shaderID = shaderID;
